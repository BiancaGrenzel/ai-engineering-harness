...
- Packaging: local `pip install .` installs the engine; read-only content pack ships for `harness init`.
- Project bootstrap: `harness init --profile software-engineer` materializes project content for external projects.
...
- PyPI publishing / release automation. Optional distribution step after local packaging.
