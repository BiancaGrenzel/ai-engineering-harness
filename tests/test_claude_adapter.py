#!/usr/bin/env python3
"""Tests for the Claude adapter generator.

Fixtures live under tests/adapters/claude/fixtures/ (isolated from the real
...
GENERATOR = ROOT / "adapters" / "claude" / "generate.py"
ADAPTER_YAML = ROOT / "adapters" / "claude" / "adapter.yaml"
...
class ClaudeAdapterTests(unittest.TestCase):
    ...
    def test_tool_registry_validation(self) -> None:
    ...
    def test_valid_claude_adapter_metadata(self) -> None:
        metadata = load_adapter_metadata(ADAPTER_YAML)
        self.assertEqual(metadata.name, "claude")
    ...
    def test_metadata_capability_consistency(self) -> None:
        metadata = load_adapter_metadata(ADAPTER_YAML)
        resolved = resolve_harness(self.root)
        planned, _warnings = claude_generate.build_plan(resolved, metadata)
    ...
    def test_dry_run_makes_no_changes(self) -> None:
        code = claude_generate.run(self.root, dry_run=True)
        self.assertEqual(code, 0)
        self.assertFalse((self.root / ".claude").exists())
        self.assertFalse((self.root / ".harness" / "adapters" / "claude.managed.json").exists())
    ...
    def test_conflict_reported(self) -> None:
        target = self.root / ".claude" / "skills" / "task-analysis" / "SKILL.md"
        target.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_stale_managed_output_is_removed(self) -> None:
        ...
        self.assertTrue(skill.is_file())
        manifest = json.loads(
            (self.root / ".harness" / "adapters" / "claude.managed.json").read_text(
                encoding="utf-8"
            )
        )
        self.assertNotIn(
    ...
    def test_unmanaged_stale_file_is_not_removed(self) -> None:
        ...
        self.assertEqual(code, 0)
        self.assertTrue(stale.is_file())
        self.assertEqual(stale.read_text(encoding="utf-8"), "# user kept\n")
    ...
    def test_does_not_manage_claude_md(self) -> None:
        ...
        original = "# User project CLAUDE.md\nDo not overwrite.\n"
        ...
        self.assertNotIn(".claude/CLAUDE.md", manifest["files"])
